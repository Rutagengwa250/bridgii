...
(function () {
  ...
  function setVisibilityForAuth(session) {
    ...
    const registerLinks = document.querySelectorAll('a[href$="register.html"], a[href*="/register.html"]');
    ...
    loginLinks.forEach((a) => {
      setNodeHidden(a, isLoggedIn);
      setNodeHidden(a.closest('li'), isLoggedIn);
    });
    ...
    loginLinks.forEach((a) => {
      ...
      if (parent) desktopContainers.add(parent);
    });
    registerLinks.forEach((a) => {
      const parent = a.closest('div');
      if (parent) desktopContainers.add(parent);
    ...
    desktopContainers.forEach((container) => {
      ...
      if (logoutBtn) setNodeHidden(logoutBtn, !isLoggedIn);
      if (userLabel) {
        setNodeHidden(userLabel, !isLoggedIn);
    ...
    if (mobileMenuList) {
      ...
      if (mobileLogoutWrap) setNodeHidden(mobileLogoutWrap, !isLoggedIn);
      if (mobileUserWrap) setNodeHidden(mobileUserWrap, !isLoggedIn);
